...
try:
    from tkinter import Tk, Button, Listbox, EXTENDED, END,Toplevel, Scrollbar, RIGHT, Y, X, Text, LEFT, BOTH, Frame, Entry, N, Radiobutton, W, BooleanVar, Label, E, CENTER, SINGLE
...
#lis.bind('<Tab>', lambda e: lis.select_set(lis.index(ACTIVE)))
for i in r:
    lis.insert(END,i)
lis.bind('<Down>', lambda e: lis.select_clear(lis.index(ACTIVE)))
lis.bind('<Down>', lambda e: lis.select_clear(lis.index(ACTIVE)))
lis.bind('<Down>', lambda e: lis.select_clear(lis.index(ACTIVE)))#.select_set(lis.index(ACTIVE)))
lis.bind('<Up>', lambda e: lis.select_clear(lis.index(ACTIVE)))#.select_set(lis.index(ACTIVE)))
lis.pack()
